...
def grid_search(num_samples: int, min: float = None, max: float = None, **kwargs)->Iterable:
    ...
    **YOU MAY IMPLEMENT THIS FUNCTION FOR Q5**
    ...
    values = torch.linspace(min, max, steps=num_samples)
...
def random_search(num_samples: int, distribution: str, min: float=None, max: float=None, **kwargs) -> Iterable:
    ...
    **YOU MAY IMPLEMENT THIS FUNCTION FOR Q5**
    ...
    if distribution == 'uniform':
        ...
        values = torch.exp(torch.rand(num_samples) * (torch.log(max) - torch.log(min)) + torch.log(min))
    elif distribution == 'normal':
        values = torch.randn(num_samples) * kwargs.get('std', 1) + kwargs.get('mean', 0)
    elif distribution == 'exponential':
        values = torch.FloatTensor([kwargs.get('lambda', 1)])\
                     .exponential_(num_samples).clamp_(min, max)
    else:
