        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        while True:
            ...
            for state_idx, state in enumerate(self.mdp.states):
                v = V[state_idx]
                max_value = float('-inf')
                # Iterate over actions to find the action with maximum expected value
                for action_idx, action in enumerate(self.mdp.actions):
                    expected_value = 0
                    # Calculate the expected value for each possible next state
                    for next_state_idx, next_state in enumerate(self.mdp.states):
                        expected_value += self.mdp.P[state_idx, action_idx, next_state_idx] * (
                            self.mdp.R[state_idx, action_idx, next_state_idx] +
                            self.gamma * V[next_state_idx]
                        )
                    max_value = max(max_value, expected_value)
                V[state_idx] = max_value  # Update value function for the state
                delta = max(delta, abs(v - V[state_idx]))  # Update change in value function
            if delta < theta:
                break  # Stop iteration if below threshold
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for state_idx, state in enumerate(self.mdp.states):
            max_action_idx = None
            max_value = float('-inf')
            # Find the action with maximum expected value for the state
            for action_idx, action in enumerate(self.mdp.actions):
                expected_value = 0
                # Calculate the expected value for each possible next state
                for next_state_idx, next_state in enumerate(self.mdp.states):
            ...
            policy[state_idx, max_action_idx] = 1.0  # Assign probability 1 to best action
        #raise NotImplementedError("Needed for Q1")
        return policy
...
class PolicyIteration(MDPSolver):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for state_idx, state in enumerate(self.mdp.states):
                ...
                expected_value = 0
                # Calculate the expected value for each possible next state
                action_values = np.sum(self.mdp.P[state_idx] * (self.mdp.R[state_idx] + self.gamma * V), axis=1)
                expected_value = np.sum(policy[state_idx] * action_values)
                V[state_idx] = expected_value # Update value function
                delta = max(delta, abs(v - V[state_idx])) # Update change in value function
            if delta < self.theta:
                break # Stop iteration if below threshold
        return V
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        ### PUT YOUR CODE HERE ###
        #raise NotImplementedError("Needed for Q1")
        policy = np.ones([self.state_dim, self.action_dim], dtype=float) / self.action_dim # Initialize uniform policy
        while True:
            V = self._policy_eval(policy) # Evaluate policy
            new_policy = np.zeros_like(policy)
            action_values = self.mdp.P * (self.mdp.R + self.gamma * np.tile(V, (self.action_dim, 1))) # Expected values of each action in each state
            policy_indices = [np.argmax(np.sum(action_value, axis=1)) for action_value in action_values] # Best actions
            for state_idx, new_policy_idx in enumerate(policy_indices):
                new_policy[state_idx, new_policy_idx] = 1
            if np.array_equal(new_policy, policy):
                break # Stop if policy stable
